...
但存在例外，`utils.api` 的类 `ProtocolEndApi` 不需要受“方法须通用”的规则限制，所有请求协议端 API（CQHTTP.AsyncApi）的操作均应调用此类包装过后的方法。
...
- 底层接口不应试图捕获未知错误并返回默认值，应当仅对自身可处理的错误进行捕获处理。
...
插件已接替了框架的错误处理，因此代码中不应捕获任何未知异常，建议直接写裸代码，使其被框架捕获即可。\
当然，我们遵循 EAFP 风格，你仍然可以大量运用 try-except。
