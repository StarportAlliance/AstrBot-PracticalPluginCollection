...
该层仅作薄包装，不含任何实际逻辑，此处应当将其转发至中层的业务方法。
不应在此层添加错误处理与日志输出。
...
所有业务核心逻辑方法应当仅返回以下其一值：
- `MessageEventResult`：消息事件结果。仅 Command Handler，return 给顶层，顶层再 yield 供 AstrBot 消费；
- `None`：不返回任何内容，仅供顶层 Event Listener 调用。
...
对于非业务核心逻辑，不应捕获异常仅是为了日志后重抛，也不应在发生异常时试图返回默认值。
